...
def test_parse_frontmatter_basic(tmp_path):
    ...
    result = bm._parse_frontmatter(md)
...
def test_parse_frontmatter_no_frontmatter(tmp_path):
    ...
    assert bm._parse_frontmatter(md) is None
...
def test_parse_frontmatter_missing_file(tmp_path):
    ...
    assert bm._parse_frontmatter(tmp_path / "nope.md") is None
...
def fake_plugins(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bm, "PLUGINS_CACHE", cache)
    # Clear cache so tests see fresh state
    bm._agent_types_cache.clear()
    bm._agent_types_cache_ts = 0.0
...
def test_scan_agent_types_cached(fake_plugins):
    ...
    first = bm._scan_agent_types()
    second = bm._scan_agent_types()
...
def test_resolve_qualified_name(fake_plugins):
    ...
    result = bm._resolve_agent_type("helioy-tools:backend-engineer")
    assert result is not None
...
def test_resolve_short_name_priority(fake_plugins):
    ...
    result = bm._resolve_agent_type("backend-engineer")
    assert result is not None
...
def test_resolve_unique_short_name(fake_plugins):
    ...
    result = bm._resolve_agent_type("code-reviewer")
    assert result is not None
...
def test_resolve_unknown_returns_none(fake_plugins):
    ...
    assert bm._resolve_agent_type("nonexistent-agent") is None
...
def test_warroom_discover_query_filter(fake_plugins):
    ...
    assert all("backend" in a["name"].lower() or "backend" in a.get("summary", "").lower()
...
def test_warroom_spawn_with_mocked_tmux(fake_plugins, monkeypatch):
    ...
    with bm.db() as conn:
        ...
        members = conn.execute("SELECT * FROM warroom_members WHERE warroom_id = 'eng'").fetchall()
...
def test_warroom_kill_removes_from_db(monkeypatch):
    ...
    with bm.db() as conn:
        ...
        conn.execute(
            "INSERT INTO warrooms (warroom_id, tmux_session, tmux_window, cwd, created_at, status) VALUES (?, ?, ?, ?, ?, ?)",
            ("test-wr", "main", "test-wr", "/tmp", bm._now(), "active"),
        )
        conn.execute(
            "INSERT INTO warroom_members (warroom_id, agent_type, tmux_target, pane_id, spawned_at) VALUES (?, ?, ?, ?, ?)",
            ("test-wr", "helioy-tools:backend-engineer", "main:1.0", "%10", bm._now()),
        )
    ...
    with bm.db() as conn:
        assert conn.execute("SELECT * FROM warrooms WHERE warroom_id = 'test-wr'").fetchone() is None
        assert conn.execute("SELECT * FROM warroom_members WHERE warroom_id = 'test-wr'").fetchall() == []
...
def test_warroom_status_cross_references_agents(monkeypatch):
    ...
    with bm.db() as conn:
        ...
        conn.execute(
            "INSERT INTO agents (agent_id, cwd, tmux_target, pid, registered_at, last_seen) VALUES (?, ?, ?, ?, ?, ?)",
